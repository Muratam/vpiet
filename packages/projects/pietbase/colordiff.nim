...
proc toConsole*(pietMap: Matrix[PietColor]): string =
  ...
  for y in 0..<pietMap.height:
    for x in 0..<pietMap.width:
      ...
      let c = case color:
        of WhiteNumber:
          getColor6(5, 5, 5).toBackColor() &
            getColor6(3, 3, 3).toForeColor() & '-'
        of BlackNumber:
          getColor6(0, 0, 0).toBackColor() &
            getColor6(2, 2, 2).toForeColor() & ' '
        else:
...
proc ciede2000*(l1, a1, b1, l2, a2, b2: float): float{.
    importc: "CIEDE2000"header: "../packages/projects/pietbase/ciede2000.h".}

var ciede2000Table = newSeqWith(maxColorNumber+1, newSeqWith(maxColorNumber+1,
    -1))
...
proc toNearlestPietColor*(r, g, b: uint8): PietColor =
  result = 0.PietColor
  var minVal = 1e12.int
  for c in 0..maxColorNumber:
    let (l1, a1, b1) = (r, g, b).rgb2xyz().xyz2lab()
    let (l2, a2, b2) = c.PietColor.toRGB().rgb2xyz().xyz2lab()
    let distance = ciede2000(l1, a1, b1, l2, a2, b2).int
    if minVal > distance:
      minVal = distance
      result = c.PietColor
