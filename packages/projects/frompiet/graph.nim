  ...
  proc search(index: int, dp: DP, cc: CC): int =
    ...
    of Switch: # 2方向
      for i in 0..<2:
        ...
        to &= (currentNodeIndex, nextNodeIndex, order, i, indexTo.blockSize[
            index])
        cc.toggle()
    of Pointer: # 4方向
      for i in 0..<4:
    ...
    of Terminate, ErrorOrder:
      maxNodeIndex += 1
      to &= (currentNodeIndex, maxNodeIndex, order, -1, indexTo.blockSize[
          index])
    else:
      let nextNodeIndex = search(nextIndex, dp, cc)
      to &= (currentNodeIndex, nextNodeIndex, order, -1, indexTo.blockSize[
          index])
      ...
      for i in 0..<self.getMaxNodeIndex():
        ...
        for a in 0..<outEdgesIndexs[i].len():
          for b in (a+1)..<outEdgesIndexs[i].len():
            if not isSame(self[outEdgesIndexs[i][a]], self[outEdgesIndexs[i][
                b]], false):
              ok = false
        if not ok: continue
    ...
    block:
      for i in 0..<self.len():
        for j in (i+1)..<self.len():
          ...
          if (self[i].dst != self[j].dst) and (self[i].orderAndSizes[
              ^1].order != Terminate): continue
          self[j].src = -1
...
proc mergeBridge(self: var seq[Edge]): bool =
  ...
  proc merge(self: var seq[Edge]): bool =
    ...
    for i in 0..<self.getMaxNodeIndex():
      ...
      self[oe].orderAndSizes = self[ie].orderAndSizes & self[
          oe].orderAndSizes
