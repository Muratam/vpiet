...
proc stegano1D*(orders: seq[PasmOrder], base: Matrix[PietColor]) =
  ...
  const maxFund = 10
  var dp = newSeqWith(chromMax + 1, newSeqWith(base.width,
      newSeqWith(base.width, newSeq[DPVal]())))
  ...
  for progress in 0..<(base.width-1):
    ...
    for nop in 0..progress:
      ...
      proc d(color, dNop, dOrd, f: int): DPKey = (color, nop+dNop, ord+dOrd,
          f)
      # 命令を進めた
      for i in 0..<chromMax:
...
proc quasiStegano1D*(orders: seq[PasmOrder], base: Matrix[PietColor]) =
  ...
  type Val = tuple[val: int, mat: Matrix[PietColor], x, y: int, dp: DP,
      cc: CC, fund: int]
  ...
  block: # 最初は白以外
    ...
    for i in 0..<chromMax:
      ...
      fronts[0] &= (distance(color, i.PietColor), initMat, 0, 0, newDP(),
          newCC(), 0)
  for progress in 0..<(base.width-1):
    ...
    for ord in 0..<fronts.len():
      ...
      proc push(f: Val, nextColor, dx, dy, dOrd, dFund: int, dp: DP, cc: CC) =
        var next: Val = (f.val, f.mat.deepCopy(), f.x + dx, f.y + dy, dp, cc,
            f.fund+dFund)
    ...
    fronts = nexts.mapIt:
      var res = it.sorted((a, b)=>a.val-b.val)[
        0..(min(it.len(), maxFrontierNum)-1)]
      res
  block:
    let front = fronts[^1]
