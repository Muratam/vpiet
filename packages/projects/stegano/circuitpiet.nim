...
proc toPiet*(self:seq[seq[PasmOrder]]) :Matrix[PietColor]=
  let maxFunLen = self.mapIt(it.filterIt(not (it.pasmType in[Goto,Goto])).len()).max()
  ...
  for y,orders in self:
    ...
    # 分岐
    var jumpOrder = ErrorPasmType
    var jumpArgs = newSeq[int]()
    for x,order in orders:
      if order.pasmType in [MoveTerminate,Goto,GoIf]:
        jumpOrder = order.pasmType
    ...
    nowColor = nowColor.searchNextPietColor(Order.Push)
    pietMap[width - 1,2 + y * 4] = nowColor
    pietMap[width - 1,3 + y * 4] = nowColor
    nowColor = nowColor.searchNextPietColor(Order.Pointer)
    ...
    case jumpOrder :
      ...
      of GoIf:
        ...
        nowColor = nowColor.searchNextPietColor(Order.Not)
        ...
        pietMap[1+2*maxArg,y2] = 0.PietColor.searchNextPietColor(Order.Pointer)
        if jumpArgs[0] > jumpArgs[1]:
          pietMap[width - 2,4 + y * 4] = nowColor.searchNextPietColor(Order.Not)
      else: discard
  # 左端上
  ...
  for y,orders in self:
    ...
    setMap(0,y,nowColor)
    for x,order in orders:
      if order.pasmType in [MoveTerminate,Goto,GoIf]: continue
      let pietOrder = order.order
      nowColor = nowColor.searchNextPietColor(pietOrder)
