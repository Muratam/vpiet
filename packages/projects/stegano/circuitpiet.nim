...
proc toPiet*(self: seq[seq[PasmOrder]]): Matrix[PietColor] =
  let maxFunLen = self.mapIt(it.filterIt(not (it.pasmType in [Goto,
      Goto])).len()).max()
  ...
  for y, orders in self:
    ...
    case jumpOrder:
      ...
      of GoIf:
        ...
        pietMap[1+2*maxArg, y2] = 0.PietColor.searchNextPietColor(
            Order.Pointer)
        if jumpArgs[0] > jumpArgs[1]:
          pietMap[width - 2, 4 + y * 4] = nowColor.searchNextPietColor(
              Order.Not)
      else: discard
  # 左端上
