...
from projects/ncurses/ncurses as nc import nil
# CUIでごにょごにょする際のラッパーモジュール

proc timeout(t: int): void {.
  cdecl, discardable, importc: "timeout", dynlib: nc.libncurses
.}
func code*[T](key: T): string = fmt "\e[{key}m"
func toForeColor*(i: int): string = code(fmt"38;5;{i}")
func toBackColor*(i: int): string = code(fmt"48;5;{i}")
func getColor6*(r, g, b: range[0..6]): int = 16 + 36 * r + 6 * g + b # all in  0 to 6
func getGray24*(gray: range[0..24]): int = 232 + gray # in 0 to 24
let
  toBold* = code(1)
  toDim* = code(2)
  toUnderline* = code(4)
...
func getKeys*(curse: var Curse): seq[char] =
  result = @[]
